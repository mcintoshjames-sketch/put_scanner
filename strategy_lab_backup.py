...
with tabs[4]:
    ...
    if strat_choice == "CSP":
        base = df_csp.copy()
        if base.empty:
            st.info("No CSP rows. Run a scan.")
        else:
            base["Key"] = (base["Ticker"] + " | " + base["Exp"] + " | K=" + base["Strike"].astype(str)
                           + " | Prem=" + base["Premium"].astype(str))
            pick = st.selectbox("Pick a CSP contract", base["Key"].tolist(), index=0)
            row = base[base["Key"] == pick].iloc[0]

            # Params for MC
            iv = float(row["IV"]) / 100.0 if row.get("IV", float("nan")) == row.get("IV", float("nan")) else 0.20
            params = dict(
                S0=float(row["Price"]),
                days=int(row["Days"]),
                iv=iv,
        ...
        base = df_cc.copy()
        if base.empty:
            st.info("No CC rows. Run a scan.")
        else:
            base["Key"] = (base["Ticker"] + " | " + base["Exp"] + " | K=" + base["Strike"].astype(str)
                           + " | Prem=" + base["Premium"].astype(str))
            pick = st.selectbox("Pick a CC contract", base["Key"].tolist(), index=0)
            row = base[base["Key"] == pick].iloc[0]
        ...
        base = df_collar.copy()
        if base.empty:
            st.info("No Collar rows. Run a scan.")
        else:
            base["Key"] = (base["Ticker"] + " | " + base["Exp"]
                           + f" | Kc={base['CallStrike'].astype(str)}"
                           + f" | Kp={base['PutStrike'].astype(str)}"
                           + f" | Net={base['NetCredit'].astype(str)}")
            pick = st.selectbox("Pick a Collar structure", base["Key"].tolist(), index=0)
            row = base[base["Key"] == pick].iloc[0]

            # Use call IV (proxy); or 20% fallback
            iv = float("nan")
            # Not stored individually; conservative default:
            iv = 0.20
            div_ps_annual = float(row.get("DivAnnualPS", 0.0)) if "DivAnnualPS" in row else 0.0
    ...
    if "mc" in locals():
        ...
        # Percentiles we already have in mc
        summary_rows = [
...
# --- Tab 7: Plan & Runbook ---
with tabs[6]:
    st.header("Plan & Runbook — Evaluate Fit & Create Orders")
    st.caption("Select a strategy and a specific contract/structure. We’ll check it against best practices and generate exact order tickets and triggers.")

    # --- Select strategy & row (reuse pattern from MC tab) ---
    colA, colB, colC = st.columns(3)
    with colA:
        strat_choice_rb = st.selectbox("Strategy", ["CSP","CC","COLLAR"], key="rb_strategy")
    with colB:
    ...
    if strat_choice_rb == "CSP":
        base = st.session_state["df_csp"].copy()
    elif strat_choice_rb == "CC":
        base = st.session_state["df_cc"].copy()
    else:
        base = st.session_state["df_collar"].copy()

    if base.empty:
        st.info("No rows for the selected strategy. Run a scan.")
    else:
        if strat_choice_rb == "COLLAR":
            base["Key"] = (base["Ticker"] + " | " + base["Exp"]
                           + " | Kc=" + base["CallStrike"].astype(str)
                           + " | Kp=" + base["PutStrike"].astype(str))
        else:
            base["Key"] = base["Ticker"] + " | " + base["Exp"] + " | K=" + base["Strike"].astype(str)

        pick = st.selectbox("Pick a specific contract/structure", base["Key"].tolist(), index=0, key="rb_pick")
        row = base[base["Key"] == pick].iloc[0]
        ...
        div_y = 0.0
        try:
            # If present on the row, prefer its dividend info
        ...
        runbook_text = build_runbook(
            ...
            risk_rules={},  # reserved for future per-user rules
...
# --- Tab 8: Stress Test ---
with tabs[7]:
    st.header("Stress Test — Mark-to-Market Shocks")
    st.caption("Apply price and IV shocks, reduce time by a horizon, and see leg-level and total P&L. Assumes Black–Scholes with dividend yield (q).")

    col1, col2, col3, col4 = st.columns(4)
    with col1:
        strat_st = st.selectbox("Strategy", ["CSP","CC","COLLAR"], key="stress_strategy")
    with col2:
    ...
    shocks_text = st.text_input("Price shocks (%) comma-separated", value="-20,-10,-5,0,5,10,20",
                                key="stress_shocks_text")
    # Parse shocks
    def _parse_shocks(s):
        out = []
        for tok in s.split(","):
    ...
    # Choose row to stress
    if strat_st == "CSP":
        base = st.session_state["df_csp"].copy()
        key_series = (base["Ticker"] + " | " + base["Exp"] + " | K=" + base["Strike"].astype(str)) if not base.empty else pd.Series([], dtype=str)
    elif strat_st == "CC":
        base = st.session_state["df_cc"].copy()
        key_series = (base["Ticker"] + " | " + base["Exp"] + " | K=" + base["Strike"].astype(str)) if not base.empty else pd.Series([], dtype=str)
    else:
        base = st.session_state["df_collar"].copy()
        if not base.empty:
            key_series = (base["Ticker"] + " | " + base["Exp"]
                          + " | Kc=" + base["CallStrike"].astype(str)
                          + " | Kp=" + base["PutStrike"].astype(str))
        else:
            key_series = pd.Series([], dtype=str)

    if base.empty:
        st.info("No rows for the selected strategy. Run a scan first.")
    else:
        pick = st.selectbox("Pick a contract/structure", key_series.tolist(), index=0, key="stress_pick")
        row = base[key_series == pick].iloc[0]

        # Dividend yield proxy (q)
        try:
            div_y = float(row.get("DivYld%", 0.0)) / 100.0
        except Exception:
            div_y = 0.0

        # Convert vol point shifts to decimals
