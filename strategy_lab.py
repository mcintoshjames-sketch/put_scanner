    ...
    for exp in expirations:
        ...
        for _, r in chain.iterrows():
            ...
            prem = effective_credit(
                r.get("bid"), r.get("ask"), r.get("lastPrice"))
            ...
            score = 0.50 * roi_ann_collat + 0.30 * \
                (min(cushion_sigma, 3.0) / 3.0 if cushion_sigma ==
                 cushion_sigma else 0.0) + 0.20 * liq_score
    ...
    for exp in expirations:
        ...
        for _, r in chain.iterrows():
            ...
            if include_dividends and div_ps_annual > 0:
                roi_ann += (div_ps_annual / S) * (D / 365.0) * \
                    (365.0 / D)  # just adds dividend yield
...
def run_stress(strategy, row, *, shocks_pct, horizon_days, r, div_y,
    ...
    if strategy == "CSP":
        ...
        for sp in shocks_pct:
            ...
            ann_roi = (1.0 + cycle_roi) ** (365.0 /
                                            max(1, horizon_days if horizon_days > 0 else 1)) - 1.0
            out.append({
                "Shock%": sp, "Price": S1,
...
with st.sidebar:
    ...
    t_bill_yield = st.number_input(
        "13-week T-bill yield (annualized, decimal)", value=0.00, step=0.01, format="%.2f")
    ...
    min_poew = st.slider("Min POEW (CSP, expire worthless)",
                         0.50, 0.95, 0.65, step=0.01)
    earn_window = st.slider(
        "Earnings window (± days, CSP/CC)", 0, 14, 5, step=1)
...
with tabs[4]:
    ...
    if row is None:
        ...
        if strat_choice == "CSP":
            iv = float(row.get("IV", float("nan"))) / 100.0 if row.get("IV",
                                                                       float("nan")) == row.get("IV", float("nan")) else 0.20
            params = dict(
                S0=float(row["Price"]),
                days=int(row["Days"]),
                iv=iv,
